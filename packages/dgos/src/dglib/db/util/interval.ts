// adapted from node-interval-tree
...
function compare(objA: Uint32Array, objB:Uint32Array):number{
    for (let i=0; i<Math.min(objA.length, objB.length); i++){
        const x = objA[i]-objB[i];
        if (x) {
            return x<0?-1:1
        }
    }
    const x= objA.length - objB.length
...
function isSame(a: Interval, b:Interval) : boolean{
    return 0==compare(a.low,b.low) && 0==compare(a.high,b.high)
}

function max(a: N, b: N): N {
  return compare(a,b)<0 ? b:a
}

function height<T extends Interval, N extends number | bigint = number>(node?: Node) {
  if (node === undefined) {
    return -1
  } else {
    return node.height
...
export class Node{
  public key: N
  public max: N
  public records: Interval[] = []
  public parent?: Node
  public height = 0
  public left?: Node
  public right?: Node

  constructor(public intervalTree: IntervalTree, record: Interval) {
    this.key = record.low
  ...
  */
  ...
  private _getOverlappingRecords(currentNode: Node, low: N, high: N) {
    if (currentNode.key <= high && low <= currentNode.getNodeHigh()) {
      // Nodes are overlapping, check if individual records in the node are overlapping
      const tempResults: Interval[] = []
  ...
  public search(low: N, high: N) {
    ...
    let leftSearch: Interval[] = []
    let ownSearch: Interval[] = []
    let rightSearch: Interval[] = []
  ...
  public searchExisting(low: N): Node | undefined {
    if (this === undefined) {
      return undefined
    }
  ...
  public remove(record: Interval) {
    if (this.root === undefined) {
      ...
      if (node === undefined) {
        ...
        let removedRecord: Interval | undefined
        ...
        if (isSame(node.records[0], record)) {
          ...
          if (this.root.key === node.key) {
            ...
            // root's parent role
            const rootParent = new Node(this, { low: record.low, high: record.low } as Interval)
...
export interface DataInterval<T, N extends number | bigint = number> extends Interval {
  data: Interval
}
...
export default class DataIntervalTree{
  ...
  public insert(low: N, high: N, data: Interval) {
    return this.tree.insert({ low, high, data })
  }

  public remove(low: N, high: N, data: Interval) {
...
export class InOrder
  implements IterableIterator<Interval>
{
  private stack: Node[] = []
  ...
  public next(): IteratorResult<Interval> {
    // Will only happen if stack is empty and pop is called
    if (this.currentNode === undefined) {
      return {
        done: true,
        value: undefined,
      } as any as IteratorResult<Interval>
    }

...
/* claimed to maybe be 0 copy, but seems unlikely? It's more likely that
blob.stream().getReader({ type: "byob" }).read(new Uint8Array(wasmMemory, offset, length)).

also byob doesn't work in safari. in chrome, you may as well use the file api.
...
export class Store {
    ...
    readPage(st: Store, d: DiskPointer, target: Uint32Array, then: ()=>{} ){
        var trans = this.db.transaction(["blob"], "readonly");
        var store = trans.objectStore("blob");
        store.get(d, 
        return false
    }
    writePage(st: Store, d: DiskPointer, target: Uint32Array, then: ()=>{}) {

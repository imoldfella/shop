import { createSignal, JSX, Component, createEffect, onCleanup, ParentComponent } from 'solid-js';
...
export const Splitter: ParentComponent<{
  left: () => number,
  ...
  const changeLeft = (clientX: number, clientY: number) => {
    props.setLeft(clientX);
  }

  // this is feeding css variables to the grid.
  // 
  const pane0 = () => props.children ?? [][0]
  const pane1 = () => props.children ?? [][1]
  return <div
    ref={grid}
    class=" w-full h-full min-h-0 font-sans"
    style={{
      "grid-template-columns": `${props.left()}px 12px 1fr`,
      "display": "grid",
    }}
  >
    <div class=' bg-teal-500 relative h-full w-full overflow-y-auto  flex  bg-color-black'>
      {pane0()}

    </div>
    <GridResizer ref={resizer} size={props.left} onResize={changeLeft} />
    {pane1()}

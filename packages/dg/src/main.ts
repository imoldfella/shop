...
export const readJson = (path: string) : any => JSON.parse(readString(path))
export const readString = (path: string) =>  fs.readFileSync(path, { encoding: 'utf-8' })
export const writeJson = (json: any, path: string) => fs.writeFileSync(path, JSON.stringify(json,null,2))
export const writeString = (s: string, path: string) => fs.writeFileSync(path, s, { encoding: 'utf8'})

async function convert(path: string, out: string) {

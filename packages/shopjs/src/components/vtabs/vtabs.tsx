...
export const Vtabs = () => {
    ...
    return (<div class=" cursor-pointer bg-white dark:bg-black overflow-hidden" classList={{
        ...
        <div class='  bg-black items-center relative flex top-0 w-full left-0'>
        ...
        <nav
            ...
            <For each={vtabs.root!.children}>{(e) =>
                ...
                <div class='w-full  overflow-hidden shadow-lg flex items-center text-white'><img class="flex-none rounded-md h-12 w-12 shadow m-2" src={e.icon} />

                    < div class=' flex-1 cursor-pointer' > {e.label}</div >
                    <Icon class='flex-none h-5 w-5 m-2 hover:text-white text-gray-400' path={ellipsisHorizontal} />
                </div >
            }</For ></nav ></div >)
